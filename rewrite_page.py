...
with open('f:/bet/betmatrix-ui/app/(public)/casino/game/[id]/page.tsx', 'r', encoding='utf-8') as f:
    content = f.read()

start_str = '{/* Game UI Simulation */}'
end_str = '</div> {/* End Main Content Col */}'
...
closing_str = '            </AnimatePresence>\n          </motion.div>\n        </div> {/* End Main Content Col */}'
closing_idx = content.find(closing_str)

if closing_idx == -1:
    print('Could not find closing string')
    exit(1)

post_content = content[closing_idx:]

replacement = '''{/* Game UI Simulation - Top 1% Stake Style */}
                        ...
                        <div className="flex items-center gap-3 bg-white/5 border border-white/10 px-3 py-2 rounded-xl">
                          ...
                          >
                            <motion.div 
                              ...
                              style={{ marginLeft: isMultiplayer ? (window.innerWidth < 768 ? '20px' : '24px') : '0px' }}
                            />
                          </button>
                        </div>
                      </div>
...
new_content = pre_content + replacement + '\n' + post_content

with open('f:/bet/betmatrix-ui/app/(public)/casino/game/[id]/page.tsx', 'w', encoding='utf-8') as f:
    f.write(new_content)
