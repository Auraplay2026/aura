      ...
      >
        <div className="flex items-center gap-2 font-black">
          <div className="w-5 h-5 flex flex-col justify-center gap-0.5">
            <div className="h-0.5 w-full bg-black rounded" />
            <div className="h-0.5 w-full bg-black rounded" />
            <div className="h-0.5 w-full bg-black rounded" />
          </div>
          Betslip
          <span className="bg-black text-[#FFD700] text-[10px] px-1.5 py-0.5 rounded ml-1">1</span>
        </div>
      ...
      <AnimatePresence>
        {isExpanded && (
          <motion.div
            ...
            className="bg-[#1a1b2a] border-x border-slate-800 shadow-[0_10px_30px_rgba(0,0,0,0.5)] overflow-hidden"
          >
            <div className="p-4">
              {/* Draft Bet Item */}
              <div className="bg-[#26283b] rounded-lg p-3 mb-4 relative group border border-transparent hover:border-slate-700 transition-colors">
                <button 
                  onClick={onClearBet}
                ...
                <div className="flex justify-between items-start mb-2 pr-6">
                  <div>
                    <span className="text-xs text-slate-400 font-bold uppercase tracking-wider">Winner</span>
                    <p className="text-sm font-black text-white">{draftBet.team}</p>
                  </div>
                  <div className="text-right">
                    <span className="text-lg font-black text-white">{draftBet.odds.toFixed(2)}</span>
                  </div>
                </div>
              ...
              <div className="mb-4">
                ...
                <div className="relative flex items-center bg-[#151623] rounded-lg border border-slate-800 p-1">
                  <span className="pl-3 text-slate-500 font-bold text-sm">₹</span>
                  <input
                    type="number"
                    value={stake}
                    onChange={(e) => setStake(Number(e.target.value))}
                    className="w-full bg-transparent border-none text-white font-black text-lg py-1 px-2 outline-none"
                  />
                </div>
              </div>
              ...
              <div className="flex justify-between items-center mb-4">
                <span className="text-xs font-bold text-slate-400">Total Payout</span>
              ...
              <button
                ...
                className="w-full bg-[#22c55e] hover:bg-green-500 disabled:bg-slate-700 text-black disabled:text-slate-500 font-black py-3 rounded-lg flex justify-center items-center gap-2 transition-colors uppercase tracking-widest text-sm"
              >
                {isProcessing ? (
