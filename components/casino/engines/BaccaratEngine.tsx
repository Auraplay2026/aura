...
export function BaccaratEngine({ isPlaying, betAmount = 10, onComplete, selectedTarget, setSelectedTarget }: BaccaratEngineProps) {
      ...
      try {
        ...
        if (res.ok && data.success) {
          ...
          if (winningHand === "TIE") {
            ...
            targetPlayerHand.push(
              { val: "A", suit: "♠", color: "text-slate-900", score: 1 },
              { val: "4", suit: "♦️", color: "text-red-600", score: 4 }
            ); // modulo score: 5
            targetBankerHand.push(
              { val: "9", suit: "♥️", color: "text-red-600", score: 9 },
          ...
          // Step 1: Deal Player Cards with sound
          setTimeout(() => {
            if (!isActive) return;
            playGameSound("card");
            setPlayerHand([targetPlayerHand[0], targetPlayerHand[1]]);
            
            // Step 2: Deal Banker Cards with sound
            setTimeout(() => {
              if (!isActive) return;
              playGameSound("card");
              setBankerHand([targetBankerHand[0], targetBankerHand[1]]);

              // Step 3: Announce outcome with female dealer voice
              setTimeout(() => {
                if (!isActive) return;
                setDealt(true);
                if (winningHand === "TIE") {
                  setResultMsg("Tie Hand!");
                  try { playDealerVoice("Tie hand, bets push!"); } catch {}
                } else if (winningHand === "PLAYER") {
                  setResultMsg("Player Wins!");
                  try { playDealerVoice("Player wins!"); } catch {}
                } else {
                  setResultMsg("Banker Wins!");
                  try { playDealerVoice("Banker wins!"); } catch {}
                }
  ...
  return (
    <div className="w-full max-w-7xl mx-auto px-1 sm:px-4 py-2 sm:py-4 text-slate-900 overflow-visible select-none font-sans relative">
      {/* 0. Live Dealer Studio Video Backdrop */}
      <div className="w-full mb-3 shadow-2xl">
        <LiveDealerStudio
          dealerName="Elena"
          gameTitle="Macau VIP Live Baccarat"
          roundTimeLeft={isPlaying ? 0 : 15}
          isSpinning={isPlaying}
          activePlayersCount={88}
          lastOutcome={resultMsg || "Player Wins"}
        />
      </div>

      <div className="w-full flex flex-col lg:flex-row gap-4 items-start">
        <div className="flex-1 w-full min-w-0">
          <div className="w-full h-full min-h-[460px] md:min-h-[520px] bg-gradient-to-br from-[#1c0808] via-[#3a0d0d] to-[#0f0404] rounded-3xl border-[16px] border-[#291715] shadow-[inset_0_0_100px_rgba(0,0,0,0.95),_inset_0_0_0_2px_rgba(245,158,11,0.25),0_20px_50px_rgba(0,0,0,0.6)] relative flex flex-col items-center justify-center overflow-hidden perspective-[1000px] p-4">
            ...
            {/* Table Decals */}
            <div className="absolute top-4 text-center opacity-30 select-none pointer-events-none">
              <h2 className="text-yellow-500 font-black text-xl md:text-3xl tracking-[0.3em] uppercase drop-shadow-md">MACAU VIP BACCARAT</h2>
              <span className="text-yellow-600 text-[9px] md:text-[10px] font-bold tracking-[0.5em] mt-0.5 block">COMMISSION FREE ROADWAY</span>
            </div>

            <div className="relative z-10 w-full flex flex-col md:flex-row gap-6 md:gap-16 justify-center mt-8 px-4 transform-style-3d rotate-x-[12deg]">
              
              {/* Player Section */}
              <div className="flex flex-col items-center gap-3">
                <div className="px-5 py-1 rounded-full border border-blue-500/30 bg-blue-500/10 backdrop-blur-sm shadow-[0_0_15px_rgba(59,130,246,0.2)]">
                ...
                {playerHand.length > 0 && (
                  <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="text-slate-900 font-mono font-black text-xl bg-white/80 border border-slate-700 px-6 py-1.5 rounded-full shadow-lg backdrop-blur-md">
                    {playerScore}
                  </motion.div>
                )}
              </div>
              ...
              {/* Banker Section */}
              <div className="flex flex-col items-center gap-3">
                <div className="px-5 py-1 rounded-full border border-red-500/30 bg-red-500/10 backdrop-blur-sm shadow-[0_0_15px_rgba(239,68,68,0.2)]">
            ...
            {!isPlaying && (
              <div className="mt-6 flex flex-wrap gap-3 z-20 justify-center">
                <button
            ...
            <AnimatePresence>
              {dealt && resultMsg && (
                <motion.div
                  ...
                  className="absolute bottom-6 z-50 px-10 py-3 bg-white/90 border border-yellow-500/50 shadow-[0_0_50px_rgba(234,179,8,0.3)] rounded-2xl backdrop-blur-lg"
                >
                  <span className={`font-black uppercase tracking-widest text-2xl drop-shadow-md ${
        ...
        <div className="shrink-0 w-full lg:w-[280px]">
          <LiveDealerChat dealerName="Elena" dealerTitle="VIP Baccarat Dealer" tableId="LIVE-BACCARAT-01" />
        </div>
      </div>
    </div>
  );
