        ...
        <div className="w-full z-10 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3 bg-white/80 backdrop-blur-md border border-slate-200 rounded-2xl px-4 py-2.5 shadow-md">
            ...
            <div className="text-left">
              <span className="text-[9px] text-slate-700 font-black uppercase tracking-widest block">TARGET</span>
          ...
          <div className="flex items-center gap-2 bg-white/70 border border-slate-200 rounded-2xl px-4 py-2">
            <span className="text-[9px] text-slate-700 font-bold uppercase tracking-widest block">CHANCE</span>
            <span className="text-sm font-black font-mono text-emerald-600">{winChance}%</span>
          </div>

          {/* Stake readout */}
          <div className="flex items-center gap-2 bg-white/70 border border-slate-200 rounded-2xl px-4 py-2">
            <span className="text-[9px] text-slate-700 font-bold uppercase tracking-widest block">Wager</span>
        ...
        <div className="relative z-10 w-full flex flex-col items-center justify-center flex-grow py-8 select-none">
          ...
          <motion.div
            ...
            className={`text-6xl md:text-[7.5rem] lg:text-[9.5rem] font-black font-mono tabular-nums tracking-tighter leading-none transition-colors duration-300 flex items-baseline select-none ${
              ...
              phase === "charging" ? "text-slate-600 animate-pulse" :
        ...
        <div className="w-full z-10 flex items-center justify-between border-t border-slate-100 pt-4 mt-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="p-2.5 rounded-xl bg-white hover:bg-slate-50 border border-slate-200 text-slate-600 hover:text-slate-950 transition-all shadow-sm flex items-center gap-1.5 text-xs font-black uppercase tracking-wider cursor-pointer"
              title="Daily Missions & Stats"
            >
              <Trophy className="w-4 h-4 text-amber-500 animate-pulse" />
              <span className="hidden sm:inline">Missions & Stats</span>
            </button>

            {/* Sound Toggle */}
            <button
              onClick={() => setIsMuted(prev => !prev)}
              className="p-2.5 rounded-xl bg-white hover:bg-slate-50 border border-slate-200 text-slate-700 hover:text-slate-900 transition-all shadow-sm cursor-pointer"
            >
      ...
      <AnimatePresence>
        {isRewardsOpen && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="absolute inset-0 bg-slate-50/60 backdrop-blur-md z-50 flex items-center justify-center p-4"
          >
            <motion.div
              initial={{ scale: 0.9, y: 15 }}
              animate={{ scale: 1, y: 0 }}
              exit={{ scale: 0.9, y: 15 }}
              className="bg-white border border-slate-200 shadow-2xl rounded-3xl w-full max-w-md p-6 text-left relative overflow-hidden"
            >
              <button
                onClick={() => setIsRewardsOpen(false)}
                className="absolute top-4 right-4 text-slate-600 hover:text-slate-600 font-black text-sm w-7 h-7 bg-slate-50 hover:bg-slate-100 rounded-full flex items-center justify-center transition-colors cursor-pointer"
              >
                ✕
              </button>
              ...
              <div className="space-y-4">
                ...
                <div className="bg-slate-50 border border-slate-200 p-4 rounded-2xl flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-indigo-500 to-cyan-500 flex items-center justify-center p-0.5 shadow-md">
                    ...
                    <div className="text-left">
                      <p className="text-[10px] text-slate-700 font-bold uppercase tracking-wider">Account Profile</p>
                      <p className="text-xs text-slate-900 font-black truncate max-w-[150px]">{email.split("@")[0]}</p>
                    </div>
                  </div>
                  <div className="bg-cyan-50 border border-cyan-200 rounded-lg px-2.5 py-1 text-center shrink-0">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="bg-slate-50 border border-slate-200 p-4 rounded-2xl text-left">
                    <span className="text-[9px] text-slate-700 font-black uppercase tracking-wider block">Session Wins</span>
                    <span className="text-lg font-black font-mono text-emerald-600">{winsCount}</span>
                  </div>
                  <div className="bg-slate-50 border border-slate-200 p-4 rounded-2xl text-left">
                    <span className="text-[9px] text-slate-700 font-black uppercase tracking-wider block">Streak</span>
                    <div className="flex items-center gap-1.5 mt-0.5">
                      <Zap className={`w-3.5 h-3.5 ${streak > 0 ? 'text-amber-500 animate-pulse' : 'text-slate-600'}`} />
                ...
                {/* Daily Missions */}
                <div className="bg-slate-50 border border-slate-200 p-4 rounded-2xl text-left relative overflow-hidden">
                  ...
                  <p className="text-xs text-slate-650 font-bold mb-3 leading-normal">
                  ...
                  <button className="w-full py-2.5 bg-gradient-to-r from-indigo-500 to-cyan-500 text-white rounded-xl text-[10px] font-black uppercase tracking-wider flex items-center justify-center gap-1 active:scale-95 transition-all shadow-md cursor-pointer">
                    Claim Milestone Rewards <ChevronRight className="w-3.5 h-3.5" />
                  </button>
                </div>
              </div>
            </motion.div>
