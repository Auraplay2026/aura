    ...
    const executeBet = async () => {
      try {
        const res = await fetch('/api/casino/bet', {
          ...
          body: JSON.stringify({
            email,
            gameId: gameId || "orig-r9",
            gameTitle: gameTitle || "Zero-Free Roulette",
        ...
        if (res.ok && data.success) {
          ...
          const targetIdx = ZERO_FREE_NUMBERS.findIndex(n => n.n === winningNumVal.n);
          if (targetIdx === -1) {
            setIsSpinning(false);
        ...
        console.error("Zero-Free Roulette bet placement failed", err);
        setIsSpinning(false);
        onCompleteRef.current(0, false);
      }
    };
