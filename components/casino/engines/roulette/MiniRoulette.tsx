    ...
    const executeBet = async () => {
      try {
        const res = await fetch('/api/casino/bet', {
          ...
          body: JSON.stringify({
            email,
            gameId: gameId || "orig-r4",
            gameTitle: gameTitle || "Mini Roulette",
        ...
        console.error("Mini Roulette bet placement failed", err);
        setIsSpinning(false);
        onCompleteRef.current(0, false);
      }
    };
              ...
              <div className="min-w-[620px] relative">
                ...
                <div className="grid grid-cols-14 border border-yellow-500/40 rounded-xl overflow-hidden bg-slate-50/40">
                  ...
                  {Array.from({ length: 4 }).map((_, colIdx) => {
                    ...
                    return (
                      <div key={`col-${colIdx}`} className="flex flex-col border-r border-yellow-500/40 w-16">
