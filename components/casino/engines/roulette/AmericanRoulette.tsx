    ...
    const executeBet = async () => {
      try {
        const res = await fetch('/api/casino/bet', {
          ...
          body: JSON.stringify({
            email,
            gameId: gameId || "orig-r2",
            gameTitle: gameTitle || "American Roulette",
        ...
        if (res.ok && data.success) {
          ...
          const targetIdx = AMERICAN_NUMBERS.findIndex(n => n.n === winningNumVal.n);
          if (targetIdx === -1) {
            setIsSpinning(false);
        ...
        console.error("American Roulette bet placement failed", err);
        setIsSpinning(false);
        onCompleteRef.current(0, false);
      }
    };
