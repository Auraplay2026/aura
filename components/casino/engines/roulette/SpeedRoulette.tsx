...
export function SpeedRoulette({ 
    ...
    const executeBet = async () => {
      try {
        const res = await fetch('/api/casino/bet', {
          ...
          body: JSON.stringify({
            email,
            gameId: gameId || "orig-r8",
            gameTitle: gameTitle || "Speed Roulette",
        ...
        if (res.ok && data.success) {
          ...
          const targetIdx = EUROPEAN_NUMBERS.findIndex(n => n.n === winningNumVal.n);
          if (targetIdx === -1) {
            setIsSpinning(false);
          ...
          // Ball falls into pocket (faster)
          setTimeout(() => {
            if (isActive) setBallRadiusOffset(0);
          }, 1200);

          // Spin completes (faster: 2000ms)
          setTimeout(() => {
            if (!isActive) return;
        ...
        console.error("Speed Roulette bet placement failed", err);
        setIsSpinning(false);
        onCompleteRef.current(0, false);
      }
    };
  ...
  return (
    <div className="w-full max-w-6xl mx-auto px-1 sm:px-4 py-2 sm:py-4 text-slate-900 overflow-visible select-none font-sans relative">
      ...
      <div className="w-full bg-white rounded-b-2xl border-x border-b border-slate-200 shadow-2xl p-3 sm:p-6 flex flex-col items-center gap-4 relative overflow-hidden">
