      ...
      {isOpen && (
        <>
          ...
          <motion.div 
            ...
            className="fixed top-0 right-0 h-full w-full max-w-lg bg-[#050914] border-l border-white/10 z-[9995] shadow-[-20px_0_100px_rgba(0,0,0,0.8)] flex flex-col overflow-hidden"
          >
            ...
            {/* Header */}
            <div className="p-6 border-b border-white/10 bg-gradient-to-b from-slate-900/80 to-black/80 flex justify-between items-center shrink-0 relative z-10 backdrop-blur-xl shadow-lg">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h2 className="text-2xl font-black text-white tracking-widest drop-shadow-md">POSITIONS</h2>
              ...
              <div className="flex items-center gap-3">
                ...
                <button onClick={onClose} className="w-10 h-10 rounded-full bg-white/5 hover:bg-white/10 border border-white/10 flex items-center justify-center transition-all hover:scale-110 active:scale-95 shadow-lg">
            ...
            <div className="p-6 shrink-0 relative z-10">
              <div className="bg-gradient-to-br from-slate-900 to-slate-950 border border-white/10 rounded-[2rem] p-6 shadow-2xl relative overflow-hidden">
                ...
                <div className="relative z-10">
                  <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-2 flex items-center gap-2">
                    <TrendingUp className="w-4 h-4 text-emerald-400" /> Total Active Value
                  </p>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="bg-black/50 rounded-2xl p-4 border border-white/5 backdrop-blur-md">
                      <div className="flex items-center gap-2 mb-2">
                        <Wallet className="w-4 h-4 text-slate-400" />
                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Invested</p>
                      </div>
                      <p className="text-xl font-black text-white font-mono">₹{totalInvested.toLocaleString()}</p>
                    </div>
                    <div className="bg-black/50 rounded-2xl p-4 border border-white/5 backdrop-blur-md relative overflow-hidden">
                      ...
                      <div className="flex items-center gap-2 mb-2 relative z-10">
                        ...
                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Total PnL</p>
                      </div>
            ...
            <div className="px-6 pb-2 shrink-0 relative z-10">
              <h3 className="text-sm font-black text-white tracking-widest uppercase flex items-center gap-2 border-b border-white/10 pb-4">
            ...
            <div className="flex-1 overflow-y-auto px-6 pb-6 custom-scrollbar relative z-10">
              {positions.length === 0 && transactions.filter(t => t.type === 'trade' && t.status === 'Pending' && t.details.startsWith('Placed')).length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full text-center py-20">
                  <div className="relative mb-8 group">
                    ...
                    <div className="w-28 h-28 rounded-full bg-gradient-to-br from-slate-900 to-[#0a0f1d] border border-white/10 flex items-center justify-center shadow-[inset_0_0_30px_rgba(0,0,0,0.8)] relative z-10 transform transition-transform duration-500 hover:scale-105">
                      <BarChart2 className="w-12 h-12 text-slate-500 group-hover:text-neon-purple transition-colors drop-shadow-md" />
                    </div>
                  </div>
                  <p className="text-2xl font-black text-white mb-3 tracking-widest drop-shadow-lg">NO ACTIVE TRADES</p>
                  <p className="text-sm text-slate-400 max-w-[280px] leading-relaxed font-medium">Explore the global markets, analyze trends, and place your first prediction to start building your empire.</p>
                  <button onClick={onClose} className="mt-8 px-8 py-3 bg-white/5 hover:bg-white/10 border border-white/10 rounded-full text-xs font-black uppercase tracking-widest text-white transition-all hover:scale-105 hover:border-white/20 hover:shadow-[0_0_20px_rgba(255,255,255,0.1)]">
                    Explore Markets
                  </button>
                </div>
              ) : (
                ...
                >
                  {positions.map((pos) => {
                    ...
                    return (
                      <motion.div variants={itemVariants} key={pos.id} className="group bg-slate-900/60 backdrop-blur-xl border border-white/5 rounded-[1.5rem] p-5 hover:border-white/20 transition-all duration-300 relative overflow-hidden">
                        ...
                        <div className="flex justify-between items-start mb-4 relative z-10">
                          <div className="flex-1 pr-4">
                            ...
                            <p className="text-sm font-black text-white leading-tight line-clamp-2">{pos.marketTitle}</p>
                          </div>
                          <div className={`flex flex-col items-end shrink-0`}>
                            <span className={`text-[10px] font-black px-3 py-1.5 rounded-full uppercase tracking-widest ${pos.side === 'yes' ? 'bg-green-500/20 text-green-400 border border-green-500/30' : 'bg-red-500/20 text-red-400 border border-red-500/30'}`}>
                              {pos.side}
                            </span>
                        ...
                        <div className="flex items-center justify-between p-3 rounded-xl bg-black/60 border border-white/10 mb-4 relative z-10 shadow-inner">
                          <div>
                            <p className="text-[10px] text-slate-500 uppercase font-bold tracking-widest mb-1">Buy Price</p>
                            <p className="text-sm font-black text-slate-300 font-mono">{pos.buyPrice}¢</p>
                          </div>
                        ...
                        <div className="flex items-center gap-4 relative z-10">
                          <div className="flex-1">
                            ...
                            <p className={`text-xl font-black font-mono tracking-tight ${isProfit ? 'text-white' : 'text-slate-300'}`}>
                              ₹{currentValue.toLocaleString(undefined, { maximumFractionDigits: 2 })}
                            </p>
                          </div>
                          <button 
                            onClick={() => cashOut(pos.id, livePrice)}
                            className="px-6 py-3 bg-white/5 hover:bg-white/10 border border-white/10 rounded-xl text-xs font-black text-white uppercase tracking-widest transition-all hover:scale-105 active:scale-95 flex items-center gap-2 group/btn shadow-lg"
                          >
                            <DollarSign className="w-4 h-4 text-slate-400 group-hover/btn:text-neon-green transition-colors" /> 
                            Cash Out
                          </button>
                        </div>
                      </motion.div>
              ...
              {(() => {
                ...
                return (
                  <div className="mt-8">
                    <h3 className="text-sm font-black text-white tracking-widest uppercase flex items-center gap-2 border-b border-white/10 pb-4 mb-4">
                      <TrendingUp className="w-4 h-4 text-emerald-400" /> Active Sports Bets ({pendingSportsBets.length})
                    </h3>
                    <motion.div 
                      variants={containerVariants}
                      initial="hidden"
                    ...
                    >
                      {pendingSportsBets.map((bet) => {
                        ...
                        return (
                          <motion.div variants={itemVariants} key={bet.id} className="group bg-slate-900/60 backdrop-blur-xl border border-white/5 rounded-[1.5rem] p-5 hover:border-white/20 transition-all duration-300 relative overflow-hidden">
                            ...
                            <div className="flex justify-between items-start mb-4 relative z-10">
                              <div className="flex-1 pr-4">
                                ...
                                <p className="text-sm font-black text-white leading-tight line-clamp-2">{matchTitle}</p>
                              </div>
                              <div className="flex flex-col items-end shrink-0">
                                <span className="text-[10px] font-black px-3 py-1.5 rounded-full uppercase tracking-widest bg-blue-500/20 text-blue-400 border border-blue-500/30">
                                  {selection}
                                </span>
                              </div>
                            </div>
                            ...
                            <div className="flex items-center justify-between p-3 rounded-xl bg-black/60 border border-white/10 mb-4 relative z-10 shadow-inner">
                              <div>
                                ...
                                <p className="text-sm font-black text-white font-mono">₹{bet.amount.toLocaleString()}</p>
                              </div>
                              <div className="w-px h-8 bg-white/10" />
                              <div>
                                <p className="text-[10px] text-slate-500 uppercase font-bold tracking-widest mb-1">Odds</p>
                                <p className="text-sm font-black text-emerald-400 font-mono">{odds}</p>
                              </div>
                              <div className="w-px h-8 bg-white/10" />
                              <div className="text-right">
                                <p className="text-[10px] text-slate-500 uppercase font-bold tracking-widest mb-1">To Win</p>
                                <p className="text-sm font-black text-white font-mono">₹{payout.toLocaleString(undefined, { maximumFractionDigits: 2 })}</p>
                              </div>
                            </div>
                            ...
                            <div className="flex items-center justify-between relative z-10">
                               <div className="flex items-center gap-2">
                                 ...
                                 <span className="text-xs font-bold text-yellow-400 bg-yellow-500/10 px-3 py-1 rounded-full border border-yellow-500/20 flex items-center gap-1.5">
                               ...
                               <button 
                                  ...
                                  className="px-4 py-2 bg-red-500/10 hover:bg-red-500/20 border border-red-500/30 rounded-xl text-[10px] font-black text-red-400 uppercase tracking-widest transition-all hover:scale-105 active:scale-95 flex items-center gap-1.5 group/btn"
                               >
                                 <X className="w-3 h-3 group-hover/btn:text-white transition-colors" /> 
                                 Cancel Bet
                               </button>
                            </div>
                          </motion.div>
