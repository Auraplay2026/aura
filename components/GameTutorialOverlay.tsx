  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        className="absolute inset-0 z-50 flex items-center justify-center bg-white/80 p-4"
      >
        <motion.div
        ...
        >
          ...
          <div className="p-8 md:p-10">
            {/* Title Section */}
            <div className="mb-8">
              <p className="text-green-600 font-mono text-xs uppercase tracking-widest mb-2 flex items-center gap-2">
                ...
                <div key={index} className="flex items-start gap-4">
                  ...
                  <p className="text-slate-700 leading-relaxed font-medium">
                    ...
                    {step.split('₹').map((part, i, arr) => (
                      <span key={i}>
                        {part}
                        {i < arr.length - 1 && <span className="text-emerald-600 font-bold">₹</span>}
                      </span>
                    ))}
                  </p>
                </div>
            ...
            <div className="flex flex-col gap-6">
              <label className="flex items-center gap-3 cursor-pointer group">
                <div className={`w-5 h-5 rounded border flex items-center justify-center transition-colors ${dontShowAgain ? 'bg-emerald-500 border-emerald-500' : 'bg-slate-50 border-slate-300 group-hover:border-slate-400'}`}>
                  {dontShowAgain && <CheckCircle2 className="w-3 h-3 text-slate-900" strokeWidth={4} />}
                </div>
                <input 
                  type="checkbox" 
                  className="hidden" 
