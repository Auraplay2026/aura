...
const CRYPTOS = [
  ...
  { id: "usdt", name: "Tether", symbol: "USDT", color: "bg-[#26A17B]/20 text-[#26A17B]" },
];
      ...
      {isOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <motion.div
            ...
            className="relative w-full max-w-md bg-slate-950 border border-slate-800 rounded-3xl shadow-2xl overflow-hidden"
          >
            {/* Header */}
            <div className="p-6 border-b border-slate-900 flex justify-between items-center bg-slate-900/30">
              ...
              <button 
                onClick={onClose}
                className="p-2 text-slate-400 hover:text-white bg-slate-900 rounded-full hover:bg-slate-800 transition-colors"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="flex bg-slate-900 p-1 rounded-xl">
                <button 
                  ...
                  className={`flex-1 py-2 text-sm font-bold rounded-lg flex items-center justify-center gap-2 transition-all ${activeTab === "crypto" ? "bg-slate-800 text-white shadow-md" : "text-slate-500 hover:text-slate-300"}`}
                >
                  <Bitcoin className="w-4 h-4" /> Crypto
                </button>
                <button 
                  onClick={() => setActiveTab("fiat")}
                  className={`flex-1 py-2 text-sm font-bold rounded-lg flex items-center justify-center gap-2 transition-all ${activeTab === "fiat" ? "bg-slate-800 text-white shadow-md" : "text-slate-500 hover:text-slate-300"}`}
                >
              ...
              {activeTab === "crypto" ? (
                    ...
                    {CRYPTOS.map((c) => (
                      <button 
                        ...
                        className={`flex flex-col items-center gap-2 p-3 rounded-xl border transition-all ${
                          ...
                          ? "bg-slate-900 border-neon-green shadow-[0_0_15px_rgba(34,197,94,0.1)]" 
                      ...
                      >
                        ...
                        <span className="text-xs font-bold text-slate-300">{c.symbol}</span>
                      </button>
                    ))}
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center gap-2 bg-slate-900 p-2 rounded-xl border border-slate-800">
                      <input 
                        ...
                        readOnly 
                        value={walletAddress}
                        className="flex-1 bg-transparent border-none text-sm text-slate-300 font-mono focus:ring-0 px-2 truncate"
                      />
                      <button 
                        onClick={handleCopy}
                        className={`p-2 rounded-lg font-bold text-xs flex items-center gap-1 transition-all ${copied ? "bg-neon-green text-slate-950" : "bg-slate-800 text-white hover:bg-slate-700"}`}
                      >
                ...
                <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300 text-center py-8">
                  ...
                  <h3 className="text-lg font-bold text-white mb-2">Buy Crypto with Fiat</h3>
                  <p className="text-sm text-slate-400 max-w-[250px] mx-auto mb-6">
