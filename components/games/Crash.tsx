  ...
  return (
    <div className="flex flex-col lg:flex-row gap-6 w-full h-[600px] bg-slate-50 rounded-2xl overflow-hidden border border-slate-200 shadow-2xl">
      ...
      <div className="w-full lg:w-80 bg-slate-50/50 p-6 flex flex-col gap-6 border-r border-slate-200 shrink-0 overflow-y-auto custom-scrollbar">
        <div>
          ...
          <div className="flex bg-white rounded-xl border border-slate-200 p-1">
            ...
            />
            <button disabled={gameState === "playing" || gameState === "cashed_out"} onClick={() => setBetAmount(Math.max(1, betAmount / 2))} className="px-3 py-2 bg-slate-50 hover:bg-slate-100 text-slate-700 rounded-lg text-xs font-bold disabled:opacity-50">1/2</button>
            <div className="w-[1px] bg-slate-100 mx-1"></div>
            <button disabled={gameState === "playing" || gameState === "cashed_out"} onClick={() => setBetAmount(betAmount * 2)} className="px-3 py-2 bg-slate-50 hover:bg-slate-100 text-slate-700 rounded-lg text-xs font-bold disabled:opacity-50">2x</button>
          </div>
        </div>
      ...
      <div className={`flex-1 relative flex flex-col items-center justify-center p-8 transition-colors duration-500 overflow-hidden ${
        gameState === "crashed" && !cashoutAt ? "bg-red-100" :
        gameState === "cashed_out" ? "bg-green-100" : "bg-slate-50"
      }`}>
        ...
        <motion.div 
          className={`text-[5rem] md:text-[8rem] font-black drop-shadow-2xl tabular-nums tracking-tighter z-10 ${
            ...
            gameState === "cashed_out" ? "text-neon-green" : "text-slate-900"
          }`}
        >
        ...
        {gameState === "crashed" && (
          <div className="text-red-500 font-bold text-2xl uppercase tracking-widest mt-4 z-10 bg-white/80 px-6 py-2 rounded-full border border-red-500/30">Crashed @ {crashPoint.toFixed(2)}x</div>
        )}
