  ...
  return (
    <div className="flex flex-col lg:flex-row gap-6 w-full max-w-6xl mx-auto h-[600px] bg-slate-50 rounded-2xl overflow-hidden border border-slate-200 shadow-2xl">
      ...
      <div className="w-full lg:w-80 bg-slate-50/50 p-6 flex flex-col gap-6 border-r border-slate-200 shrink-0 overflow-y-auto custom-scrollbar">
        ...
        <div>
          <label className="text-sm font-bold text-slate-600 mb-2 block">Outside Bets (2x)</label>
          <div className="grid grid-cols-2 gap-2 mb-2">
            ...
            <button onClick={() => setSelectedBet("black")} disabled={isSpinning} className={`py-2 rounded bg-slate-100 text-slate-700 font-bold border ${selectedBet === "black" ? "border-slate-400" : "border-transparent"}`}>Black</button>
          </div>
          <div className="grid grid-cols-2 gap-2 mb-2">
            <button onClick={() => setSelectedBet("even")} disabled={isSpinning} className={`py-2 rounded bg-white text-slate-700 font-bold border ${selectedBet === "even" ? "border-slate-500" : "border-slate-200"}`}>Even</button>
            <button onClick={() => setSelectedBet("odd")} disabled={isSpinning} className={`py-2 rounded bg-white text-slate-700 font-bold border ${selectedBet === "odd" ? "border-slate-500" : "border-slate-200"}`}>Odd</button>
          </div>
          <div className="grid grid-cols-2 gap-2">
            <button onClick={() => setSelectedBet("1-18")} disabled={isSpinning} className={`py-2 rounded bg-white text-slate-700 font-bold border ${selectedBet === "1-18" ? "border-slate-500" : "border-slate-200"}`}>1 to 18</button>
            <button onClick={() => setSelectedBet("19-36")} disabled={isSpinning} className={`py-2 rounded bg-white text-slate-700 font-bold border ${selectedBet === "19-36" ? "border-slate-500" : "border-slate-200"}`}>19 to 36</button>
          </div>
        </div>
        ...
        <div className="mt-auto">
          <button 
            ...
            className={`w-full py-4 rounded-xl font-black text-lg transition-all transform active:scale-95 shadow-lg ${
              ...
              !selectedBet ? "bg-slate-100 text-slate-500 cursor-not-allowed" :
      ...
      <div className="flex-1 relative flex flex-col items-center justify-center p-4 bg-white overflow-hidden">
        ...
        <div className="relative w-48 h-48 sm:w-64 sm:h-64 mb-8">
          ...
          <motion.div
            ...
            className="w-full h-full rounded-full border-[12px] border-slate-200 bg-[conic-gradient(red_0deg,red_180deg,black_180deg,black_360deg)] relative shadow-2xl"
          >
            {/* Inner Hub */}
        ...
        <AnimatePresence>
          {result !== null && !isSpinning && (
            ...
            >
              <div className={`w-8 h-8 rounded flex items-center justify-center text-slate-900 ${getNumberColor(result)}`}>
                {result}
              </div>
        ...
        <div className="w-full max-w-lg mt-4 flex select-none">
          ...
          <div className="flex-1 grid grid-cols-12 gap-[1px] bg-slate-100 p-[1px]">
