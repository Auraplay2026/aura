...
export default function ClientUserAnalytics({ 
  ...
  return (
    <div className="min-h-screen bg-[#030307] text-slate-100 p-6 flex flex-col gap-6 relative">
      {/* Toast Alert */}
      {toast && (
      ...
      {/* Header bar */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-slate-900/30 border border-slate-800/80 rounded-2xl p-6 backdrop-blur-md">
        <div>
          ...
          <h1 className="text-2xl font-black text-white mt-1 tracking-tight">Player Profile Controls & Analytics</h1>
          <p className="text-xs text-slate-400 mt-0.5">Audit wagers turnover, approve KYC documents, make balance overrides, and save reviewer logs.</p>
        </div>
        <div className="flex items-center gap-3 bg-slate-950/60 border border-slate-900 px-4 py-2.5 rounded-xl">
          <Database className="w-5 h-5 text-indigo-400" />
          <span className="text-[10px] font-mono text-slate-400">
            Database Snapshot: <span className="text-emerald-400">users.json</span>
          </span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-4 bg-slate-900/20 border border-slate-800/60 rounded-2xl p-5 backdrop-blur-md flex flex-col gap-4">
          <h2 className="font-bold text-sm text-slate-300 uppercase tracking-wider flex items-center gap-2 px-1">
            <User className="w-5 h-5 text-violet-400" /> Player Directory
          </h2>
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full bg-slate-950/80 border border-slate-800 rounded-xl pl-10 pr-4 py-2.5 text-xs focus:outline-none focus:border-violet-500/80 focus:ring-1 focus:ring-violet-500/30 transition text-slate-200"
            />
          </div>

          <div className="flex flex-col gap-2 max-h-[580px] overflow-y-auto pr-1 custom-scrollbar">
            {filteredUsers.length === 0 ? (
              <div className="text-center py-8 text-xs text-slate-500">No players found</div>
            ) : (
                  ...
                  <button
                    ...
                    className={`w-full text-left p-3.5 rounded-xl border transition duration-200 flex flex-col gap-1.5 cursor-pointer relative overflow-hidden group ${
                      isSelected 
                        ...
                        : "bg-slate-950/40 border-slate-900/60 hover:bg-slate-900/60 hover:border-slate-800"
                    }`}
                  >
                    {isSelected && (
                    ...
                    <div className="flex items-center justify-between">
                      <span className="font-bold text-slate-200 text-xs group-hover:text-violet-300 transition-colors">
                        {user.username}
                      </span>
                      <span className={`text-[9px] px-2 py-0.5 rounded font-semibold border ${
                        user.role === 'BANNED' ? "bg-rose-950/50 text-rose-300 border-rose-900/50" :
                        user.role === 'admin' ? "bg-amber-950/50 text-amber-300 border-amber-900/50" :
                    ...
                    <div className="flex items-center justify-between text-[10px] mt-1 pt-1.5 border-t border-slate-900/40">
                      <span className="text-slate-500">
                        Turnover: <span className="text-slate-300 font-mono font-bold">₹{totalWagers.toLocaleString('en-IN')}</span>
                      </span>
        ...
        <div className="lg:col-span-8 flex flex-col gap-6">
          {!activeUser ? (
            <div className="bg-slate-900/20 border border-slate-800/50 rounded-2xl p-16 text-center backdrop-blur-md">
              <User className="w-16 h-16 text-slate-600 mx-auto mb-4 animate-pulse" />
              <h3 className="text-xl font-bold text-slate-300">No User Selected</h3>
            ...
            <>
              ...
              <div className="bg-slate-900/30 border border-slate-800/80 rounded-2xl p-6 backdrop-blur-md flex flex-col md:flex-row md:items-center justify-between gap-4 relative overflow-hidden">
                ...
                <div className="flex items-start gap-4">
                  <div className="w-12 h-12 rounded-xl bg-violet-950/60 border border-violet-500/30 flex items-center justify-center shrink-0">
                    <User className="w-6 h-6 text-violet-400" />
                  </div>
                  <div>
                    <div className="flex flex-wrap items-center gap-2">
                      <h2 className="text-xl font-black text-slate-100">{activeUser.username}</h2>
                      ...
                      {activeUser.kycStatus === 'APPROVED' ? (
                        <span className="text-[9px] bg-cyan-950/50 text-cyan-300 border border-cyan-800/50 px-2.5 py-0.5 rounded-full font-semibold tracking-wide uppercase">
                          KYC Verified
                        </span>
                      ) : activeUser.kycStatus === 'PENDING' ? (
                        <span className="text-[9px] bg-amber-950/50 text-amber-300 border border-amber-800/50 px-2.5 py-0.5 rounded-full font-semibold tracking-wide uppercase animate-pulse">
                          KYC Pending Review
                        </span>
                      ) : (
                        <span className="text-[9px] bg-slate-900/60 text-slate-500 border border-slate-800 px-2.5 py-0.5 rounded-full font-semibold tracking-wide uppercase">
                          No KYC File
                        </span>
                      )}
                    </div>
                    <p className="text-xs text-slate-400 font-mono mt-0.5">{activeUser.email}</p>
                  </div>
                </div>

                <div className="flex flex-col items-end gap-1 shrink-0">
                  <div className="text-[10px] text-slate-500 uppercase tracking-wider font-semibold">Real Wallet Balance</div>
              ...
              {isTurnoverWarning && (
                <div className="bg-amber-950/25 border border-amber-500/30 rounded-xl p-4 flex items-start gap-3 text-amber-300">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {[
                  { label: "Total Deposits", val: depositsTotal, color: "text-slate-300" },
                  { label: "Total Withdrawals", val: withdrawalsTotal, color: "text-slate-300" },
                  { label: "Total Wagered", val: wagersTotal, color: "text-violet-400" },
                  { label: "Wager Turnover", val: `${turnoverRatio.toFixed(1)}%`, color: isTurnoverWarning ? 'text-amber-400' : 'text-emerald-400' },
                ].map((stat) => (
                  <div key={stat.label} className="bg-slate-900/30 border border-slate-800/60 rounded-xl p-4 backdrop-blur-sm">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="bg-slate-900/30 border border-slate-800/60 rounded-2xl p-5 backdrop-blur-md flex flex-col gap-4">
                  <h3 className="font-bold text-xs text-slate-300 uppercase tracking-wider flex items-center gap-2 pb-2 border-b border-slate-800/40">
                    <Activity className="w-4 h-4 text-violet-400" /> Gaming Metrics
                  </h3>

                  <div className="grid grid-cols-2 gap-4">
                    <div className="bg-slate-950/50 border border-slate-900 p-3 rounded-lg">
                      <div className="text-[9px] text-slate-500 font-semibold uppercase">Win Rate</div>
                    ...
                    <div className="bg-slate-950/50 border border-slate-900 p-3 rounded-lg">
                      <div className="text-[9px] text-slate-500 font-semibold uppercase">Total Payouts</div>
                    ...
                    <div className="bg-slate-950/50 border border-slate-900 p-3 rounded-lg">
                      <div className="text-[9px] text-slate-500 font-semibold uppercase">Avg Bet Size</div>
                      <div className="text-base font-bold font-mono text-slate-200 mt-1">
                    ...
                    <div className="bg-slate-950/50 border border-slate-900 p-3 rounded-lg">
                      <div className="text-[9px] text-slate-500 font-semibold uppercase">Net House Profit</div>
                      <div className={`text-base font-bold font-mono mt-1 ${netHouseProfit >= 0 ? 'text-indigo-400' : 'text-rose-400'}`}>
                  ...
                  <div className="flex flex-col gap-2.5 mt-2 flex-grow justify-end">
                    <div className="text-[10px] text-slate-400 font-semibold uppercase tracking-wider">Favorite Games Distribution</div>
                    {sortedGames.length === 0 ? (
                      <div className="text-xs text-slate-500 italic py-2">No gameplay records found</div>
                    ) : (
                          ...
                          return (
                            <div key={game.name} className="flex flex-col gap-1">
                              <div className="flex justify-between text-[10px] font-medium">
                                <span className="capitalize text-slate-300">{game.name}</span>
                                <span className="text-slate-400 font-mono">{game.count} rounds ({percent.toFixed(0)}%)</span>
                              </div>
                ...
                <div className="bg-slate-900/30 border border-slate-800/60 rounded-2xl p-5 backdrop-blur-md flex flex-col gap-4">
                  <h3 className="font-bold text-xs text-slate-300 uppercase tracking-wider flex items-center gap-2 pb-2 border-b border-slate-800/40">
                    <FileText className="w-4 h-4 text-violet-400" /> Auditor & Override Control Panel
                  </h3>

                  {/* Quick Specs */}
                  <div className="grid grid-cols-2 gap-2 text-[10px] bg-slate-950/40 p-2.5 rounded-xl border border-slate-900">
                    ...
                    <div className="flex items-center gap-1.5 text-slate-400 truncate">
                      ...
                      <span>Wallet: <span className="font-bold capitalize text-slate-300">{activeUser.accountType}</span></span>
                    </div>
                  </div>

                  {/* Quick Controls Grid */}
                  <div className="flex flex-col gap-2 border-t border-slate-800/40 pt-3 mt-1">
                    <div className="text-[9px] text-slate-500 font-extrabold uppercase tracking-widest">Financial Overrides</div>
                    <div className="grid grid-cols-3 gap-2">
                      <button
                        onClick={() => { setAmountInput(""); setCreditModalOpen(true); }}
                        disabled={activeUser.role === 'BANNED'}
                        className="bg-emerald-500/10 hover:bg-emerald-500 border border-emerald-500/20 hover:border-emerald-500/50 text-emerald-400 hover:text-slate-950 font-bold py-2 rounded-lg text-[10px] uppercase tracking-wider transition cursor-pointer text-center disabled:opacity-30"
                      >
                        Credit
                      </button>
                      <button
                        ...
                        className="bg-slate-900 hover:bg-slate-800 border border-slate-800 text-slate-300 hover:text-white font-bold py-2 rounded-lg text-[10px] uppercase tracking-wider transition cursor-pointer text-center disabled:opacity-30"
                      >
                        Debit
                      </button>
                      <button
                        onClick={() => { setAmountInput(activeUser.realBalance.toString()); setOverrideModalOpen(true); }}
                        className="bg-indigo-500/15 hover:bg-indigo-500 border border-indigo-500/20 hover:border-indigo-500/50 text-indigo-400 hover:text-white font-bold py-2 rounded-lg text-[10px] uppercase tracking-wider transition cursor-pointer text-center"
                      >
                        Override
                      </button>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-2 mt-1">
                      <button
                        ...
                        className={`font-bold py-2 rounded-lg text-[10px] uppercase tracking-wider transition cursor-pointer text-center border ${
                          activeUser.role === 'BANNED'
                            ? "bg-emerald-500/5 hover:bg-emerald-500 border-emerald-500/15 text-emerald-400 hover:text-slate-950"
                            : "bg-rose-500/5 hover:bg-rose-500 border-rose-500/15 text-rose-400 hover:text-white"
                        }`}
                      >
                      ...
                      {activeUser.kycStatus === 'PENDING' ? (
                        <div className="flex gap-1.5 shrink-0">
                          <button
                            ...
                            className="bg-cyan-600 hover:bg-cyan-500 text-white font-bold py-2 px-2.5 rounded-lg text-[9px] uppercase tracking-wider transition cursor-pointer flex-1"
                          >
                            Verify KYC
                          </button>
                          <button
                            onClick={() => { setDeclineReason(""); setKycDeclineModalOpen(true); }}
                            className="bg-rose-500/10 hover:bg-rose-600 border border-rose-500/20 text-rose-400 hover:text-white font-bold py-2 px-2.5 rounded-lg text-[9px] uppercase tracking-wider transition cursor-pointer flex-1"
                          >
                            Reject
                          </button>
                        </div>
                      ) : (
                        <div className="bg-slate-950/40 border border-slate-900 rounded-lg py-2 text-center text-[9px] font-black text-slate-500 uppercase tracking-widest">
                  ...
                  {activeUser.kycDocumentUrl && (
                    <div className="flex items-center justify-between text-[10px] bg-indigo-500/5 border border-indigo-500/10 p-2 rounded-lg">
                      ...
                      <a 
                        ...
                        target="_blank" 
                        rel="noreferrer"
                        className="text-[9px] text-white bg-indigo-600 hover:bg-indigo-500 px-2 py-1 rounded font-bold underline"
                      >
                        Open Attachment
                      </a>
                    </div>
                  ...
                  {/* Auditor notes input */}
                  <div className="flex flex-col gap-1.5 mt-1 border-t border-slate-800/40 pt-3">
                    <div className="flex items-center justify-between">
                      <label className="text-[10px] text-slate-400 font-semibold uppercase tracking-wider">Auditor Notes</label>
                      <button
                        onClick={handleSaveNotes}
                        disabled={isPending}
                        className="text-[9px] font-black text-violet-400 hover:text-violet-300 uppercase tracking-widest flex items-center gap-1 cursor-pointer disabled:opacity-35"
                      >
                    ...
                    <textarea
                      ...
                      className="w-full bg-slate-950/70 border border-slate-800 rounded-xl p-3 text-[10px] focus:outline-none focus:border-violet-500/80 focus:ring-1 focus:ring-violet-500/30 transition text-slate-200 resize-none h-[80px]"
                    />
                  </div>
              ...
              <div className="bg-slate-900/20 border border-slate-800/60 rounded-2xl p-5 backdrop-blur-md flex flex-col gap-4">
                <h3 className="font-bold text-xs text-slate-300 uppercase tracking-wider flex items-center gap-2 pb-2 border-b border-slate-800/40">
                  <Activity className="w-4 h-4 text-violet-400" /> Recent Round & Transaction Activity
                </h3>
                  ...
                  <table className="w-full text-left border-collapse text-xs">
                    ...
                    <tbody>
                      {sortedTxHistory.length === 0 ? (
                        <tr>
                          <td colSpan={6} className="text-center py-6 text-slate-500 italic">No transactions found</td>
                        </tr>
                      ) : (
                        sortedTxHistory.slice(0, 10).map((tx) => {
                          ...
                          return (
                            <tr key={tx.id} className="border-b border-slate-900/60 hover:bg-slate-900/20 transition-colors">
                              <td className="py-2.5 px-3 font-mono text-slate-400">{date}</td>
                              <td className="py-2.5 px-3 font-mono text-slate-300 font-semibold">{tx.id}</td>
                              ...
                              <td className="py-2.5 px-3 text-slate-300 max-w-xs truncate">{tx.details}</td>
      ...
      <div className="bg-slate-900/30 border border-slate-800/60 rounded-2xl overflow-hidden backdrop-blur-md transition-all">
        <button
          onClick={() => setLogsExpanded(!logsExpanded)}
          className="w-full px-5 py-4 flex items-center justify-between hover:bg-slate-900/20 transition cursor-pointer"
        >
          <div className="flex items-center gap-3">
            <Bell className="w-5 h-5 text-violet-400" />
            <div className="text-left">
              <h3 className="font-bold text-sm text-slate-200">Backend Notifications Dispatch Feed</h3>
              <p className="text-[11px] text-slate-500">SMTP and Twilio SMS manual deposit triggers audit ({notificationLogs.length} logged dispatches)</p>
            </div>
          </div>
        ...
        {logsExpanded && (
          <div className="border-t border-slate-800/80 p-5 bg-slate-950/20 max-h-[400px] overflow-y-auto custom-scrollbar">
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse text-xs">
                <thead>
                  <tr className="border-b border-slate-850 text-slate-500 font-semibold">
                ...
                <tbody>
                  {notificationLogs.length === 0 ? (
                    ...
                    [...notificationLogs].reverse().map((log) => {
                      ...
                      const emailStatusColor = log.emailDispatch.status === 'SUCCESS' ? 'bg-emerald-950 text-emerald-400 border-emerald-900/30' :
                        log.emailDispatch.status === 'FAILED' ? 'bg-rose-950 text-rose-400 border-rose-900/30' : 'bg-slate-900 text-slate-500 border-slate-800';
                      const smsStatusColor = log.smsDispatch.status === 'SUCCESS' ? 'bg-emerald-950 text-emerald-400 border-emerald-900/30' :
                        log.smsDispatch.status === 'FAILED' ? 'bg-rose-950 text-rose-400 border-rose-900/30' : 'bg-slate-900 text-slate-500 border-slate-800';
                      ...
                      return (
                        <tr key={log.id} className="border-b border-slate-900/60 hover:bg-slate-900/20 transition-colors">
                          ...
                          <td className="py-2.5 px-3 font-mono text-slate-300 font-semibold">{log.userEmail}</td>
                          <td className="py-2.5 px-3 font-mono font-bold text-emerald-400">₹{log.amount.toLocaleString('en-IN')}</td>
                          <td className="py-2.5 px-3 font-mono text-slate-400">{log.utr}</td>
      ...
      <AnimatePresence>
        {creditModalOpen && activeUser && (
          <div className="fixed inset-0 z-[9999] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
            <div className="bg-slate-950 border border-slate-900 w-full max-w-sm rounded-2xl p-6 shadow-2xl flex flex-col gap-4 text-left">
              <div className="flex items-center justify-between">
                <h3 className="font-extrabold text-sm uppercase text-slate-200">Inject Wallet Credit</h3>
                <button onClick={() => setCreditModalOpen(false)} className="text-slate-500 hover:text-white cursor-pointer"><X className="w-5 h-5" /></button>
              </div>
              <p className="text-xs text-slate-400 leading-relaxed">
                Inject real funds into <span className="text-white font-bold">{activeUser.username}</span>'s wallet. Current balance: <span className="text-emerald-400 font-bold font-mono">₹{activeUser.realBalance.toLocaleString()}</span>.
              </p>
              
              <div>
                <label className="block text-[9px] font-bold text-slate-500 uppercase mb-1.5">Amount (₹)</label>
                <input
                  type="number"
                  placeholder="0.00"
                  value={amountInput}
                  onChange={(e) => setAmountInput(e.target.value)}
                  className="w-full bg-slate-900 border border-slate-850 rounded-xl py-3 px-4 text-xs text-white focus:outline-none focus:border-violet-500 transition-colors font-mono"
                />
              </div>
              ...
              <div className="grid grid-cols-4 gap-1.5">
                {[1000, 5000, 10000, 50000].map(val => (
                  <button
                    ...
                    className="bg-slate-900 hover:bg-slate-800 border border-slate-850 py-1.5 rounded-lg font-mono text-[10px] text-slate-400 cursor-pointer"
                  >
      ...
      <AnimatePresence>
        {debitModalOpen && activeUser && (
          <div className="fixed inset-0 z-[9999] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
            <div className="bg-slate-950 border border-slate-900 w-full max-w-sm rounded-2xl p-6 shadow-2xl flex flex-col gap-4 text-left">
              <div className="flex items-center justify-between">
                <h3 className="font-extrabold text-sm uppercase text-slate-200">System Balance Debit</h3>
                <button onClick={() => setDebitModalOpen(false)} className="text-slate-500 hover:text-white cursor-pointer"><X className="w-5 h-5" /></button>
              </div>
              <p className="text-xs text-slate-400 leading-relaxed">
                Deduct funds from <span className="text-white font-bold">{activeUser.username}</span>'s wallet. Available: <span className="text-emerald-400 font-bold font-mono">₹{activeUser.realBalance.toLocaleString()}</span>.
              </p>
              ...
              <div>
                ...
                <input
                  ...
                  value={amountInput}
                  onChange={(e) => setAmountInput(e.target.value)}
                  className="w-full bg-slate-900 border border-slate-850 rounded-xl py-3 px-4 text-xs text-white focus:outline-none focus:border-rose-500 transition-colors font-mono"
                />
              </div>
              ...
              <button
                onClick={handleDebit}
                disabled={actionLoading}
                className="w-full bg-rose-600 hover:bg-rose-500 text-white font-bold py-3 rounded-xl transition flex items-center justify-center gap-2 cursor-pointer text-xs uppercase tracking-wider disabled:opacity-50 mt-2"
              >
      ...
      <AnimatePresence>
        {overrideModalOpen && activeUser && (
          <div className="fixed inset-0 z-[9999] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
            <div className="bg-slate-950 border border-slate-900 w-full max-w-sm rounded-2xl p-6 shadow-2xl flex flex-col gap-4 text-left">
              <div className="flex items-center justify-between">
                <h3 className="font-extrabold text-sm uppercase text-slate-200">God-Mode Balance Override</h3>
                <button onClick={() => setOverrideModalOpen(false)} className="text-slate-500 hover:text-white cursor-pointer"><X className="w-5 h-5" /></button>
              </div>
              <p className="text-xs text-slate-400 leading-relaxed">
                Forcefully set <span className="text-white font-bold">{activeUser.username}</span>'s balance. This directly overwrites current values, writing a correction transaction log.
              </p>
              
              <div>
                <label className="block text-[9px] font-bold text-slate-500 uppercase mb-1.5">New Balance Target (₹)</label>
                <input
                  type="number"
                  value={amountInput}
                  onChange={(e) => setAmountInput(e.target.value)}
                  className="w-full bg-slate-900 border border-slate-850 rounded-xl py-3 px-4 text-xs text-white focus:outline-none focus:border-indigo-500 transition-colors font-mono"
                />
              </div>
      ...
      <AnimatePresence>
        {banModalOpen && activeUser && (
          <div className="fixed inset-0 z-[9999] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
            <div className="bg-slate-950 border border-rose-500/20 w-full max-w-sm rounded-2xl p-6 shadow-2xl flex flex-col gap-4 text-left">
              <div className="flex items-center justify-between">
                <h3 className="font-extrabold text-sm uppercase text-rose-400 flex items-center gap-1.5">
                  <ShieldAlert className="w-4.5 h-4.5" /> Security Clearance Required
                </h3>
                <button onClick={() => setBanModalOpen(false)} className="text-slate-500 hover:text-white cursor-pointer"><X className="w-5 h-5" /></button>
              </div>
              <p className="text-xs text-slate-400 leading-relaxed">
                Are you sure you want to {activeUser.role === 'BANNED' ? 'pardon and unban' : 'suspend and ban'} user <span className="text-white font-bold">{activeUser.username}</span>?
                {activeUser.role !== 'BANNED' && " Banned users cannot trade or log in."}
              </p>

              <div className="flex items-start gap-2 bg-slate-900/40 p-3 rounded-lg border border-slate-850">
                <input
                  type="checkbox"
                  id="banConfirm"
                ...
                />
                <label htmlFor="banConfirm" className="text-[10px] text-slate-400 leading-normal select-none cursor-pointer">
      ...
      <AnimatePresence>
        {kycDeclineModalOpen && activeUser && (
          <div className="fixed inset-0 z-[9999] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
            <div className="bg-slate-950 border border-slate-900 w-full max-w-sm rounded-2xl p-6 shadow-2xl flex flex-col gap-4 text-left">
              <div className="flex items-center justify-between">
                <h3 className="font-extrabold text-sm uppercase text-slate-200">Decline Identity Documents</h3>
                <button onClick={() => setKycDeclineModalOpen(false)} className="text-slate-500 hover:text-white cursor-pointer"><X className="w-5 h-5" /></button>
              </div>
              <p className="text-xs text-slate-400 leading-relaxed">
              ...
              <textarea
                ...
                className="w-full h-[100px] bg-slate-900 border border-slate-850 rounded-xl p-3 text-xs focus:outline-none focus:border-rose-500/80 transition resize-none text-slate-200"
              />

              <button
                onClick={submitKycDecline}
                disabled={actionLoading}
                className="w-full bg-rose-600 hover:bg-rose-500 text-white font-bold py-3 rounded-xl transition flex items-center justify-center gap-2 cursor-pointer text-xs uppercase tracking-wider disabled:opacity-50 mt-1"
              >
