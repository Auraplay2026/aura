...
export default function ClientAffiliateDashboard() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Header */}
      <div className="bg-slate-900/40 border border-slate-800/80 rounded-3xl p-6 backdrop-blur-xl relative overflow-hidden flex justify-between items-center">
        ...
        <div>
          ...
          <p className="text-sm text-slate-400 mt-1 pl-4 relative z-10">Track your best promoters and their referral performance.</p>
        </div>
        <button 
          onClick={fetchLeaderboard}
          className="bg-slate-800 hover:bg-slate-700 text-white p-3 rounded-xl transition-colors border border-slate-700 relative z-10"
        >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-gradient-to-br from-slate-900 to-slate-950 border border-slate-800/80 rounded-3xl p-6 shadow-xl relative group overflow-hidden">
            ...
            <div className="flex items-center justify-between mb-4 relative z-10">
              <h3 className="text-sm font-bold text-slate-400 uppercase tracking-widest">Total Affiliates</h3>
              <div className="w-10 h-10 rounded-full bg-slate-900 border border-slate-800 flex items-center justify-center">
                <Users className="w-5 h-5 text-neon-green" />
              </div>
            </div>
            <p className="text-3xl font-black text-white relative z-10">{stats.totalAffiliates}</p>
          </div>

          <div className="bg-gradient-to-br from-slate-900 to-slate-950 border border-slate-800/80 rounded-3xl p-6 shadow-xl relative group overflow-hidden">
            <div className="absolute inset-0 bg-neon-purple/5 opacity-0 group-hover:opacity-100 transition-opacity" />
            <div className="flex items-center justify-between mb-4 relative z-10">
              <h3 className="text-sm font-bold text-slate-400 uppercase tracking-widest">Total Network Signups</h3>
              <div className="w-10 h-10 rounded-full bg-slate-900 border border-slate-800 flex items-center justify-center">
                <Trophy className="w-5 h-5 text-neon-purple" />
              </div>
            </div>
            <p className="text-3xl font-black text-white relative z-10">{stats.totalSignups}</p>
          </div>

          <div className="bg-gradient-to-br from-slate-900 to-slate-950 border border-slate-800/80 rounded-3xl p-6 shadow-xl relative group overflow-hidden">
            <div className="absolute inset-0 bg-yellow-500/5 opacity-0 group-hover:opacity-100 transition-opacity" />
            <div className="flex items-center justify-between mb-4 relative z-10">
              <h3 className="text-sm font-bold text-slate-400 uppercase tracking-widest">Total Commissions</h3>
              <div className="w-10 h-10 rounded-full bg-slate-900 border border-slate-800 flex items-center justify-center">
                <Coins className="w-5 h-5 text-yellow-500" />
              </div>
            </div>
            <p className="text-3xl font-black text-white relative z-10">₹ {stats.totalCommissions.toLocaleString()}</p>
          </div>
        </div>
      )}
      ...
      <div className="bg-slate-900/40 border border-slate-800/80 rounded-3xl p-6 backdrop-blur-xl">
        <h2 className="text-lg font-black text-white mb-6 uppercase tracking-wider">Top Promoters Leaderboard</h2>
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-slate-800/50">
                {promoters.map((promoter, idx) => (
                  <tr key={promoter.username} className="group hover:bg-slate-800/30 transition-colors">
                    ...
                    <td className="py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <div className="text-[10px] text-slate-500">{promoter.email}</div>
                        </div>
                      </div>
                    </td>
                    <td className="py-4">
                      <code className="bg-slate-950 border border-slate-800 px-2 py-1 rounded text-xs text-neon-green font-mono">
                        {promoter.affiliateCode}
                      </code>
                    </td>
                    <td className="py-4 text-right">
                      <span className="inline-flex items-center justify-center px-2.5 py-1 rounded-full bg-slate-800 text-xs font-bold text-white">
