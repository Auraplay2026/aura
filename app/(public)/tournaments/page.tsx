...
export default function TournamentsPage() {
  ...
  let tierBorder = "border-slate-800";
  ...
  const [leaderboard, setLeaderboard] = useState([
    { rank: 1, name: "NinjaPlayz", score: 9842100, tier: "Grandmaster 👑", winRate: "78%", avatar: "https://images.unsplash.com/photo-1566492031525-8782986cd23e?w=100&q=80", color: "text-amber-400", bg: "bg-amber-400/10", border: "border-amber-400" },
    { rank: 2, name: "Shroud_X", score: 9102450, tier: "Grandmaster 👑", winRate: "75%", avatar: "https://images.unsplash.com/photo-1542751371-adc38448a05e?w=100&q=80", color: "text-slate-300", bg: "bg-slate-300/10", border: "border-slate-300" },
    ...
    { rank: 4, name: "TimTheTat", score: 8100500, tier: "Diamond I 💎", winRate: "68%", avatar: "https://images.unsplash.com/photo-1552820728-8b83bb6b773f?w=100&q=80", color: "text-cyan-400", bg: "bg-cyan-400/10", border: "border-cyan-400/50" },
  ...
  return (
    <div className="w-full max-w-[1600px] mx-auto p-4 sm:p-6 lg:p-8 space-y-10 font-sans pb-32 relative">
      ...
      {/* Stats Hype Bar */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 bg-[#0a0c16]/80 border border-slate-900 rounded-2xl p-5 shadow-2xl relative z-10 backdrop-blur-xl">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-lg font-black text-white font-mono">{onlinePlayers.toLocaleString()}</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-lg font-black text-green-400 font-mono">{formatCurrency(totalWageredToday)}</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-lg font-black text-white font-mono">1.82M+</p>
          </div>
        </div>
      </div>
      
      {/* Hero Header */}
      <div className="relative w-full h-[280px] sm:h-[350px] rounded-3xl overflow-hidden border border-white/5 flex items-center justify-center text-center shadow-3xl">
        <div className="absolute inset-0 bg-[#050505]">
          ...
          <div className="absolute inset-0 bg-gradient-to-t from-[#020205] via-[#050508]/85 to-transparent" />
        ...
        <div className="relative z-10 flex flex-col items-center max-w-3xl px-4">
          ...
          <motion.h1 initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ delay: 0.1 }} className="text-4xl sm:text-6xl font-black text-white uppercase tracking-tighter transform -skew-x-3 mb-3">
          ...
          <motion.p initial={{ y: 15, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.2 }} className="text-slate-400 text-sm sm:text-base font-medium max-w-xl leading-relaxed">
      ...
      <div className="flex flex-col lg:flex-row gap-8 relative z-10">
        ...
        <div className="flex-1 space-y-6">
          ...
          <div className="flex items-center gap-6 border-b border-slate-900 pb-0.5 relative">
            <button 
              onClick={() => setActiveTab("leaderboard")}
              className={`pb-4 px-2 text-xs font-black uppercase tracking-wider relative transition-colors ${activeTab === "leaderboard" ? "text-amber-500" : "text-slate-500 hover:text-slate-300"}`}
            >
            ...
            <button 
              ...
              className={`pb-4 px-2 text-xs font-black uppercase tracking-wider relative transition-colors ${activeTab === "tournaments" ? "text-cyan-400" : "text-slate-500 hover:text-slate-300"}`}
            >
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
              ...
              <div className="space-y-3">
                ...
                {isRanked && (
                  <motion.div 
                    ...
                    className={`flex items-center justify-between p-4.5 rounded-2xl border ${tierBorder} bg-[#0e1222] shadow-[0_0_20px_rgba(234,179,8,0.15)] ring-1 ring-yellow-500/20 relative overflow-hidden`}
                  >
                    ...
                    <div className="flex-1 flex items-center gap-4">
                      <div className={`w-12 h-12 rounded-xl overflow-hidden border-2 ${tierBorder} bg-[#050914] flex items-center justify-center`}>
                    ...
                    <div className="w-32 justify-center hidden sm:flex">
                      <div className="flex items-center gap-1.5 bg-slate-900/60 px-3 py-1.5 rounded-lg border border-white/5">
                        ...
                        <span className="text-slate-300 font-bold text-sm">70%</span>
                      </div>
                    </div>

                    <div className="w-40 text-right">
                      <span className="text-xl sm:text-2xl font-black text-white font-mono tracking-tight">{userScore.toLocaleString()}</span>
                ...
                {leaderboard.map((player, idx) => (
                  <motion.div 
                    key={player.rank}
                    className={`flex items-center justify-between p-4 rounded-2xl border ${player.border} bg-slate-950/80 shadow-lg hover:bg-white/[0.02] transition-colors group relative overflow-hidden`}
                  >
                    <div className="w-16 flex justify-center">
                    ...
                    <div className="w-32 justify-center hidden sm:flex relative z-10">
                      <div className="flex items-center gap-1.5 bg-slate-900/60 px-3 py-1.5 rounded-lg border border-white/5">
                        <Target className="w-4 h-4 text-neon-green" />
                        <span className="text-slate-300 font-bold text-sm">{player.winRate}</span>
                      </div>
                    </div>

                    <div className="w-40 text-right relative z-10">
                      <span className="text-xl sm:text-2xl font-black text-white font-mono tracking-tight">{player.score.toLocaleString()}</span>
                      <p className="text-[9px] font-bold text-slate-500 uppercase tracking-widest">RP Points</p>
                    </div>
                  </motion.div>
                ))}
              ...
              <div className="mt-8 flex justify-center">
                <button 
                  ...
                  className="px-6 py-3.5 bg-slate-900 hover:bg-slate-800 text-white font-bold uppercase tracking-wider rounded-xl border border-white/10 transition-colors flex items-center gap-2 text-xs"
                >
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {[
                ...
                <motion.div 
                  ...
                  className="bg-[#05060f] rounded-3xl border border-slate-900 overflow-hidden group hover:border-cyan-500/50 transition-colors shadow-2xl"
                >
                  <div className="h-44 relative overflow-hidden">
                    <div className="absolute top-4 left-4 z-20 flex gap-2">
                      <span className="bg-red-500 text-white text-[9px] font-black uppercase tracking-widest px-3 py-1 rounded-full flex items-center gap-1.5">
                        <span className="w-1.5 h-1.5 rounded-full bg-white animate-pulse" /> Live Now
                      </span>
                    </div>
                    <div className="absolute inset-0 bg-gradient-to-t from-[#05060f] via-[#05060f]/40 to-transparent z-10" />
                  ...
                  <div className="p-6 relative z-20 -mt-6">
                    <h3 className="text-xl font-black text-white uppercase tracking-tight mb-4">{tourney.title}</h3>
                    ...
                    <div className="grid grid-cols-2 gap-4 mb-5">
                      <div className="bg-slate-950 rounded-xl p-3 border border-slate-900">
                        ...
                        <p className="text-lg font-black text-green-400 font-mono">{tourney.prize}</p>
                      </div>
                      <div className="bg-slate-950 rounded-xl p-3 border border-slate-900">
                        <p className="text-[9px] font-bold text-slate-500 uppercase tracking-widest mb-0.5">Active Players</p>
                        <p className="text-lg font-black text-white font-mono flex items-center gap-1.5"><Users className="w-4 h-4 text-cyan-400" /> {tourney.players}</p>
                      </div>
                    </div>
        ...
        <div className="w-full lg:w-96 shrink-0 space-y-6">
          ...
          {/* Your Rank Card */}
          <div className="bg-[#060814] border border-slate-900 rounded-3xl p-6 shadow-2xl relative overflow-hidden">
            <div className="absolute top-0 right-0 w-32 h-32 bg-cyan-500/5 blur-[50px] rounded-full pointer-events-none" />
            <h3 className="text-white font-black uppercase tracking-widest text-[10px] flex items-center gap-2 mb-6">
            ...
            <div className="flex flex-col items-center justify-center text-center mb-6">
              {isRanked ? (
                <>
                  ...
                  <h4 className="text-xl font-black text-white uppercase tracking-tighter">{userTier}</h4>
                  <p className="text-slate-400 font-bold text-xs mt-1">Platform Rank: #{userRank}</p>
                </>
              ) : (
                <>
                  <div className="w-20 h-20 rounded-full bg-slate-900 border-2 border-slate-800 flex items-center justify-center mb-3.5 shadow-lg">
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center text-xs font-bold">
                ...
                <span className="text-cyan-400 font-mono">{placementProgress} / 5</span>
              </div>
              ...
              {!isRanked ? (
                <p className="text-[10px] text-slate-500 leading-normal text-center bg-black/40 border border-slate-900 p-2.5 rounded-lg mt-2">
              ...
              <button 
                ...
                className="w-full mt-2 py-3 bg-white/5 hover:bg-white/10 text-white font-black uppercase tracking-wider rounded-xl border border-white/5 transition-colors text-xs"
              >
                Go to Arcade Lobby
              </button>
            </div>
          ...
          <div className="bg-[#060814] border border-slate-900 rounded-3xl p-6 shadow-2xl relative overflow-hidden">
            <h3 className="text-white font-black uppercase tracking-widest text-[10px] flex items-center gap-2 mb-5">
              <Activity className="w-4 h-4 text-purple-400" /> Platform Arena Activity
            </h3>
              ...
              <AnimatePresence initial={false}>
                {liveFeed.map(feed => (
                  <motion.div 
                    ...
                    className={cn(
                      "flex items-center justify-between p-3 rounded-xl border bg-black/40 text-[11px] font-bold",
                      feed.isWin ? "border-emerald-500/10" : "border-slate-900"
                    )}
                  >
                    <div>
                      <span className="text-slate-300 font-mono select-all">@{feed.user}</span>
                      <p className="text-slate-500 text-[10px] mt-0.5">
                        {feed.action} on <span className="text-white">{feed.game}</span>
                      </p>
                    </div>
                    <div className="text-right">
                      <span className={cn(
                        "font-mono font-black",
                        feed.isWin ? "text-emerald-400" : "text-[#a855f7]"
                      )}>
                        {feed.amount}
                      </span>
          ...
          {/* Winning Strategies */}
          <div className="bg-[#060814] border border-slate-900 rounded-3xl p-6 shadow-2xl">
            <h3 className="text-white font-black uppercase tracking-widest text-[10px] flex items-center gap-2 mb-5">
            ...
            <div className="space-y-4">
              {[
                ...
                <div key={i} className="flex items-start gap-3.5 p-3 rounded-xl bg-black/40 border border-slate-900">
                  <div className="mt-0.5">{strat.icon}</div>
                  <div>
                    <h4 className="text-white font-bold text-xs">{strat.title}</h4>
...
function CrownIcon() {
  return (
    <svg className="w-4.5 h-4.5 text-yellow-400 drop-shadow-[0_0_8px_rgba(250,204,21,0.8)]" viewBox="0 0 24 24" fill="currentColor">
      <path d="M2 4h20v2H2z" />
