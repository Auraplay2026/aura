    ...
    <div className="min-h-screen bg-slate-900 text-slate-100 flex flex-col items-center justify-center p-6 font-sans">
      <div className="w-full max-w-md bg-slate-950 border border-slate-800 rounded-3xl p-8 shadow-2xl relative overflow-hidden">
        ...
        <div className="flex flex-col items-center text-center mb-8">
          ...
          <p className="text-sm text-slate-400 mt-1">to continue to <span className="text-yellow-500 font-extrabold">AuraPlay</span></p>
        </div>
            ...
            <div className="space-y-2.5">
              {MOCK_ACCOUNTS.map((acc) => (
                <button
                  ...
                  className="w-full flex items-center justify-between p-4 bg-slate-900 hover:bg-slate-900/60 border border-slate-800 hover:border-slate-700 rounded-2xl text-left transition-all group"
                >
                  <div>
                    <p className="text-sm font-bold text-white group-hover:text-yellow-500 transition-colors">{acc.name}</p>
              ...
              <button
                ...
                className="w-full flex items-center gap-3 p-4 bg-white/5 hover:bg-white/10 border border-dashed border-slate-800 hover:border-slate-600 rounded-2xl text-left transition-all group"
              >
                ...
                <span className="text-sm font-bold text-slate-300 group-hover:text-white transition-colors">Use another account</span>
              </button>
            </div>

            <div className="mt-8 pt-4 border-t border-slate-900 flex items-center justify-center gap-2 text-xs text-slate-500">
        ...
        {step === 'custom' && (
          <form onSubmit={handleCustomSubmit} className="space-y-4">
            ...
            <div>
              <label className="block text-xs font-bold text-slate-400 mb-1.5 ml-1">Google Email</label>
              <div className="relative">
            ...
            <div className="flex gap-3 mt-6">
              <button
                ...
                className="flex-1 bg-slate-900 hover:bg-slate-900/60 border border-slate-800 text-slate-300 font-bold py-3 rounded-xl transition-colors"
              >
                Back
              </button>
              <button
        ...
        {step === 'loading' && (
          <div className="flex flex-col items-center justify-center py-12">
            <span className="w-12 h-12 border-4 border-slate-800 border-t-blue-500 rounded-full animate-spin mb-4" />
            <p className="text-sm font-bold text-white">Connecting Google Account...</p>
            <p className="text-xs text-slate-400 mt-1">{selectedAccount?.email}</p>
          </div>
        )}
      </div>
    </div>
