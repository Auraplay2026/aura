      ...
      <div className="bg-slate-900/40 border border-slate-800/80 rounded-3xl p-8 backdrop-blur-xl relative overflow-hidden shrink-0">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-end justify-between gap-8">
          <div>
            ...
            <p className="text-slate-400 max-w-xl text-lg">Track your active prediction shares and manage your financial exposure.</p>
          </div>
        ...
        <div className="grid grid-cols-1 gap-4">
          {HOLDINGS.map((holding, i) => (
            <motion.div
              ...
              className="bg-slate-900/40 border border-slate-800/80 rounded-2xl p-6 hover:bg-slate-900/80 transition-all duration-300 backdrop-blur-md flex flex-col lg:flex-row lg:items-center justify-between gap-6"
            >
              <div className="flex-1">
                <h3 className="text-lg font-bold text-white mb-2">{holding.market}</h3>
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-sm text-slate-400 font-medium">{holding.shares} Shares</span>
                </div>
              </div>

              <div className="flex flex-wrap items-center gap-8 bg-slate-950/50 p-4 rounded-xl border border-slate-800/50 lg:w-auto">
                <div>
                  <p className="text-xs text-slate-500 font-bold uppercase tracking-wider mb-1">Avg Price</p>
                  <p className="text-white font-bold">{holding.avgPrice}¢</p>
                </div>
                <div>
                  <p className="text-xs text-slate-500 font-bold uppercase tracking-wider mb-1">Current</p>
                  <p className="text-white font-bold">{holding.currentPrice}¢</p>
                </div>
                ...
                <button className="bg-slate-800 hover:bg-slate-700 text-white text-sm font-bold py-2 px-6 rounded-lg transition-colors border border-slate-700">
                  Trade
                </button>
              </div>
            </motion.div>
