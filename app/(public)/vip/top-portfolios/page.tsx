...
export default function TopPortfoliosPage() {
  ...
  return (
    <div className="w-full min-h-screen bg-slate-50 text-slate-900 p-4 sm:p-6 lg:p-8 relative overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto mb-8">
        <div className="bg-slate-50/80 backdrop-blur-md border border-amber-500/20 rounded-xl p-3 flex items-center gap-4 overflow-hidden shadow-[0_0_30px_rgba(245,158,11,0.1)]">
          ...
          <div className="flex-1 overflow-hidden relative h-6">
            <AnimatePresence mode="wait">
              <motion.p
                ...
                className="text-sm font-bold text-slate-700 absolute w-full"
              >
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-6 mb-12 relative z-10">
          <div className="text-center md:text-left">
            <h1 className="text-4xl md:text-6xl font-black uppercase tracking-tighter mb-4 flex items-center justify-center md:justify-start gap-4">
              <Crown className="w-12 h-12 text-amber-600 drop-shadow-[0_0_15px_rgba(251,191,36,0.6)]" />
          ...
          <div className="flex gap-2 p-1.5 bg-slate-50/80 backdrop-blur-md border border-slate-200 rounded-2xl shrink-0">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12 relative z-10">
          <div className="bg-gradient-to-br from-slate-900 to-slate-50 border border-slate-200 rounded-3xl p-6 shadow-2xl relative overflow-hidden group">
            ...
            <p className="text-3xl font-black text-slate-900 font-mono">₹4.2 Billion</p>
          </div>
          <div className="bg-gradient-to-br from-slate-900 to-slate-50 border border-slate-200 rounded-3xl p-6 shadow-2xl relative overflow-hidden group">
        ...
        <div className="space-y-6 relative z-10">
          {WHALES.map((whale, idx) => (
            <motion.div
              ...
              className={`relative overflow-hidden rounded-[2rem] border ${whale.rank === 1 ? 'border-amber-500/50 shadow-[0_0_50px_rgba(245,158,11,0.15)]' : 'border-slate-200'} bg-slate-50/60 backdrop-blur-xl p-1`}
            >
              {whale.rank === 1 && (
              ...
              <div className="flex flex-col md:flex-row items-center gap-6 p-6">
                ...
                <div className="flex items-center gap-6 w-full md:w-auto">
                  <div className={`w-12 h-12 flex items-center justify-center font-black text-xl rounded-full ${whale.rank === 1 ? 'bg-amber-500 text-black' : 'bg-slate-100 text-slate-600'}`}>
                    #{whale.rank}
                  </div>
                  ...
                  <div>
                    <h2 className="text-2xl font-black text-slate-900 tracking-wide">{whale.name}</h2>
                ...
                <div className={`grid grid-cols-2 xl:grid-cols-4 gap-4 flex-1 w-full ${whale.isLocked ? 'blur-md select-none' : ''}`}>
                  <div>
                    ...
                    <p className="text-lg md:text-xl font-black font-mono text-slate-900">{whale.pnl}</p>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <p className="text-lg md:text-xl font-black font-mono text-slate-900">{whale.winRate}</p>
                  </div>
                  <div>
                    <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest mb-1 whitespace-nowrap">Followers</p>
                    <p className="text-lg md:text-xl font-black font-mono text-slate-900 flex items-center gap-1">
                      <Users className="w-4 h-4 text-slate-600" /> {whale.followers.toLocaleString()}
                    </p>
                  </div>
                </div>
                ...
                <div className="w-full md:w-auto shrink-0 flex items-center justify-center pt-4 md:pt-0">
                  {whale.isLocked ? (
                    <div className="flex flex-col items-center justify-center gap-2 px-8 py-4 bg-white rounded-2xl border border-slate-200 absolute inset-0 md:relative md:inset-auto z-20">
