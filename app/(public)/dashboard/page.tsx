...
const VIP_LEVELS = [
  ...
  { name: 'Silver', threshold: 50000, color: 'text-slate-700', bg: 'bg-slate-300', shadow: 'shadow-slate-300/20', icon: Zap, perks: ['Level Up Bonus: ₹500', 'Weekly Bonus (5%)'] },
    ...
    <div className="min-h-screen bg-[#02050a] pt-24 pb-20 selection:bg-neon-purple/30">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 bg-[#0a0f1c] border border-slate-200 rounded-3xl p-6 md:p-10 relative overflow-hidden shadow-[0_20px_50px_rgba(0,0,0,0.5)]">
            {/* VIP Glow Background */}
            ...
            <div className="flex flex-col md:flex-row items-center gap-10 relative z-10">
              ...
              <div className="relative w-48 h-48 shrink-0 flex items-center justify-center">
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center bg-[#02050a]/40 rounded-full backdrop-blur-sm m-2 border border-slate-200">
              ...
              <div className="flex-1 w-full text-center md:text-left">
                <p className="text-slate-600 text-sm font-bold uppercase tracking-widest mb-1">Total Wagered</p>
          ...
          {/* Quick Stats Card */}
          <div className="bg-[#0a0f1c] border border-slate-200 rounded-3xl p-6 relative overflow-hidden flex flex-col gap-4">
            ...
            <div className="bg-[#131b2c] p-4 rounded-2xl border border-slate-200 flex items-center justify-between">
              <div>
                <p className="text-slate-700 text-xs font-bold uppercase tracking-wider mb-1">Current Balance</p>
            ...
            <div className="bg-[#131b2c] p-4 rounded-2xl border border-slate-200">
              <p className="text-slate-700 text-xs font-bold uppercase tracking-wider mb-1">Total Bets Placed</p>
              <p className="text-xl font-black text-slate-700 font-mono">{currentUser.transactions.filter(t => t.type === 'casino' || t.type === 'trade').length}</p>
            </div>
          </div>
        </div>
              ...
              <div 
                ...
                className={`relative p-5 rounded-2xl border transition-all duration-300 overflow-hidden ${
                  isUnlocked 
                    ...
                    : 'bg-[#0a0f1c]/50 border-slate-200 opacity-60 grayscale'
                }`}
              >
                {!isUnlocked && <Lock className="absolute top-4 right-4 w-4 h-4 text-slate-700" />}
                
                <level.icon className={`w-8 h-8 mb-4 ${isUnlocked ? level.color : 'text-slate-700'}`} />
                <h3 className={`font-black uppercase tracking-widest mb-1 ${isUnlocked ? level.color : 'text-slate-600'}`}>{level.name}</h3>
                <p className="text-[10px] font-bold text-slate-700 uppercase tracking-widest mb-4 font-mono">
                  {level.threshold === 0 ? 'Unlocked' : `₹${(level.threshold / 1000)}k+ Wager`}
                </p>

                <ul className="space-y-2">
                  {level.perks.map((perk, j) => (
                    <li key={j} className="text-xs text-slate-700 font-medium flex items-start gap-1.5">
