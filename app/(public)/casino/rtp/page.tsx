      ...
      <div className="bg-slate-900/40 border border-slate-800/80 rounded-3xl p-8 backdrop-blur-xl relative overflow-hidden shrink-0">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-end justify-between gap-8">
          <div>
            ...
            <p className="text-slate-400 max-w-xl text-lg">Real-time Return To Player statistics based on millions of spins across the network in the last 24 hours.</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {RTP_GAMES.map((game, i) => (
          <motion.div
            ...
            className={`bg-slate-900/40 border rounded-3xl p-6 backdrop-blur-md relative overflow-hidden group cursor-pointer transition-all duration-300 ${
          ...
          >
            ...
            <div className="flex items-end justify-between mb-4">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-bold text-slate-400 uppercase tracking-widest">Live RTP</span>
              </div>
              <div className={`text-4xl font-black tracking-tighter ${
                game.status === 'hot' ? 'text-orange-500 drop-shadow-[0_0_10px_rgba(249,115,22,0.4)]' :
                game.status === 'cold' ? 'text-cyan-400 drop-shadow-[0_0_10px_rgba(6,182,212,0.4)]' :
                'text-white'
              }`}>
                {game.rtp}%
              </div>
