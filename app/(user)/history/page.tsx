...
export default function BetHistoryPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Header */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-slate-900/40 border border-slate-800/80 rounded-3xl p-6 backdrop-blur-xl shrink-0">
        <div>
          <h1 className="text-2xl font-black text-white flex items-center gap-3 tracking-tight">
            <span className="w-1.5 h-6 bg-neon-green rounded-full shadow-[0_0_10px_rgba(34,197,94,0.6)]"></span>
            Transaction Ledger
          </h1>
          <p className="text-sm text-slate-400 mt-1 pl-4 flex items-center gap-2">
            <Activity className="w-3 h-3 text-neon-green" /> Real-time synchronized history.
          </p>
        </div>
        <div className="flex gap-2">
          <button className="flex items-center gap-2 px-4 py-2 bg-slate-950 border border-slate-800 rounded-xl text-sm font-bold text-slate-300 hover:text-white hover:bg-slate-900 transition-colors">
            <Calendar className="w-4 h-4" /> All Time
          </button>
          <button className="flex items-center gap-2 p-2 bg-slate-950 border border-slate-800 rounded-xl text-slate-400 hover:text-white hover:bg-slate-900 transition-colors">
      ...
      {/* Tabs */}
      <div className="flex items-center gap-2 p-1 bg-slate-950 border border-slate-800/80 rounded-2xl w-max overflow-x-auto">
        {TABS.map((tab) => (
          <button
            key={tab}
          ...
          >
            {activeTab === tab && (
              <motion.div
                ...
                className="absolute inset-0 bg-slate-800 rounded-xl shadow-inner border border-slate-700"
              />
            )}
      ...
      <div className="bg-slate-900/40 border border-slate-800/80 rounded-3xl overflow-hidden backdrop-blur-xl">
        <div className="w-full overflow-x-auto custom-scrollbar">
          <table className="w-full text-left border-collapse min-w-[900px]">
            <thead>
              <tr className="text-xs uppercase tracking-widest text-slate-500 border-b border-slate-800/80 bg-slate-950/50">
                ...
                <AnimatePresence mode="popLayout">
                  {filteredTransactions.map((row) => (
                    <motion.tr
                      ...
                      exit={{ opacity: 0 }}
                      className="border-b border-slate-800/30 hover:bg-slate-800/50 transition-colors group"
                    >
                      <td className="py-4 px-6">
                        <span className="font-mono text-xs text-slate-500 group-hover:text-slate-400 transition-colors">{row.id}</span>
                      </td>
                      ...
                      <td className="py-4 px-4">
                        <div className="flex items-center gap-2">
                          ...
                          {row.type === 'trade' && <TrendingDown className="w-4 h-4 text-slate-400" />}
                          {row.type === 'cashout' && <TrendingUp className="w-4 h-4 text-neon-green" />}
                          {row.type === 'casino' && <Coins className="w-4 h-4 text-purple-500" />}
                          <span className="text-xs text-slate-300 font-bold uppercase tracking-widest">{row.type}</span>
                        </div>
                      </td>
                      <td className="py-4 px-4 text-sm font-medium text-slate-300 max-w-[300px] truncate" title={row.details}>
                        {row.details}
                      </td>
                      <td className="py-4 px-4 text-right">
                        <span className={cn(
                          "font-black tracking-tight",
                          (row.type === 'deposit' || row.type === 'cashout' || (row.type === 'casino' && row.amount > 0)) ? "text-neon-green drop-shadow-[0_0_5px_rgba(34,197,94,0.3)]" : "text-white"
                        )}>
