...
export default function ReferAndEarnPage() {
  ...
  if (!isLoggedIn) {
    return (
      <div className="bg-slate-900/40 border border-slate-800/80 rounded-3xl p-8 backdrop-blur-xl relative overflow-hidden flex flex-col items-center text-center max-w-xl mx-auto my-12">
        ...
        <h2 className="text-2xl font-black text-white uppercase tracking-tight">Referral Program Locked</h2>
        <p className="text-slate-400 mt-2 text-sm leading-relaxed max-w-sm mb-6">
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-slate-900/40 border border-slate-800/80 rounded-3xl p-6 backdrop-blur-xl relative overflow-hidden">
        ...
        <h1 className="text-2xl font-black text-white flex items-center gap-3 tracking-tight relative z-10">
          <span className="w-1.5 h-6 bg-neon-purple rounded-full shadow-[0_0_10px_rgba(168,85,247,0.6)]"></span>
          Affiliate Dashboard
        </h1>
        <p className="text-sm text-slate-400 mt-1 pl-4 relative z-10">Invite friends and earn lifetime commissions on their play.</p>
      </div>

      {/* Top Metrics Cards */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        
        <div className="bg-gradient-to-br from-slate-900 to-slate-950 border border-slate-800/80 rounded-3xl p-6 shadow-xl relative group overflow-hidden">
          <div className="absolute inset-0 bg-neon-green/5 opacity-0 group-hover:opacity-100 transition-opacity" />
          <div className="flex items-center justify-between mb-4 relative z-10">
            <h3 className="text-sm font-bold text-slate-400 uppercase tracking-widest">Commission Earned</h3>
          ...
          <p className="text-3xl font-black text-white relative z-10 drop-shadow-[0_0_10px_rgba(34,197,94,0.3)]">₹ {((currentUser?.affiliateEarnings || 0)).toLocaleString()}</p>
        ...
        <div className="bg-gradient-to-br from-slate-900 to-slate-950 border border-slate-800/80 rounded-3xl p-6 shadow-xl relative group overflow-hidden">
          ...
          <div className="flex items-center justify-between mb-4 relative z-10">
            <h3 className="text-sm font-bold text-slate-400 uppercase tracking-widest">Total Referrals</h3>
            <div className="w-10 h-10 rounded-full bg-slate-900 border border-slate-800 flex items-center justify-center">
              <Users className="w-5 h-5 text-neon-purple" />
            </div>
          </div>
          <p className="text-3xl font-black text-white relative z-10 drop-shadow-[0_0_10px_rgba(168,85,247,0.3)]">{currentUser?.referralCount || 0}</p>
        ...
        <div className="bg-gradient-to-br from-slate-900 to-slate-950 border border-slate-800/80 rounded-3xl p-6 shadow-xl relative group overflow-hidden">
          ...
          <div className="flex items-center justify-between mb-4 relative z-10">
            <h3 className="text-sm font-bold text-slate-400 uppercase tracking-widest">Current Rate</h3>
            <div className="w-10 h-10 rounded-full bg-slate-900 border border-slate-800 flex items-center justify-center">
              <Percent className="w-5 h-5 text-neon-yellow" />
            </div>
          </div>
          <p className="text-3xl font-black text-white relative z-10 drop-shadow-[0_0_10px_rgba(234,179,8,0.3)]">15%</p>
      ...
      {/* Invite Link Section */}
      <div className="bg-slate-900/40 border border-slate-800/80 rounded-3xl p-6 lg:p-10 backdrop-blur-xl flex flex-col md:flex-row items-center gap-8 shadow-2xl">
        ...
        <div className="w-full md:w-1/2 space-y-4">
          ...
          >
            ...
            <input 
              ...
              readOnly 
              value={referralLink} 
              className="w-full bg-slate-950 border border-slate-800 rounded-2xl pl-12 pr-16 py-5 text-sm text-white font-bold focus:outline-none focus:border-neon-purple focus:ring-1 focus:ring-neon-purple transition-all shadow-inner cursor-pointer"
            />
            <motion.div 
              className={cn(
              ...
              whileTap={{ scale: 0.95 }}
            >
              {copied ? <CheckCircle2 className="w-5 h-5 text-slate-950" /> : <Copy className="w-5 h-5 text-white" />}
            </motion.div>
          </div>
        </div>
      </div>
