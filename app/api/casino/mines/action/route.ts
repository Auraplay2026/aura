...
export async function POST(request: Request) {
  try {
    const { action, email, sessionId, tileIndex } = await request.json();
    ...
    if (action === 'reveal') {
      ...
      if (session.gameId === 'orig-7') {
        ...
        if (isBust) {
          ...
          const nextMultiplier = session.activeMultiplier * 1.5; // simple multiplier scaling
          session.activeMultiplier = parseFloat(nextMultiplier.toFixed(2));

          if (session.revealedRows === 8) { // top row reached
            const payout = Math.round(session.betAmount * session.activeMultiplier * 100) / 100;
    ...
    if (action === 'cashout') {
      ...
      if (isCrash) {
        const { clientMultiplier } = await request.json();
